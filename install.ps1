        ...
        if ($Lido -gt 0) {
            ...
            if ($TotalBytes -gt 0) {
                ...
                if ($Percentual -ne $UltimoPercentual) {
                    ...
                    $Barra = "[" + ("#" * $BlocosCheios) + ("." * $BlocosVazios) + "]"
...
foreach ($Arquivo in $ArquivosRelease) {
    BaixarArquivo -Url "$BaseUrl/$Arquivo" -DestinoArquivo "$Destino\$Arquivo" -Nome $Arquivo
}
...
Start-Process -FilePath "powershell.exe" -Verb RunAs -ArgumentList "-ExecutionPolicy Bypass -NoExit -File `"$Destino\instalar.ps1`""
